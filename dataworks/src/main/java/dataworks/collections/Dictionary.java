...
public class Dictionary<TKey, TValue> implements IDictionary<TKey, TValue>, Serializable
{
    ...
    private Node first;
    private Node last;
    private Comparator<TKey> keyComparator;
    ...
    public Dictionary(Comparator<TKey> comparator)
    {
        ...
        valueComparator = new DefaultComparator<>();
        version = 0;
    }
    ...
    public TValue get(TKey key)
    {
        ...
        {
            if (keyComparator.compare(current.key, key) == 0)
    ...
    public Comparator<TKey> getKeyComparator()
    {
    ...
    public boolean containsValue(TValue value)
    {
        for (Node current = first; current != null; current = current.next)
        {
            if (valueComparator.compare(current.value, value) == 0)
                return true;
        }
    ...
    public boolean containsKeyValue(TKey key, TValue value)
    {
        ...
        {
            if (keyComparator.compare(current.key, key) == 0 &&
                    valueComparator.compare(current.value, value) == 0)
                return true;
        }
    ...
    public boolean removeByKey(TKey key)
    {
        ...
        if (keyComparator.compare(first.key, key) == 0)
        {
            first = first.next;
            count--;
            version++;
        ...
        {
            if (keyComparator.compare(precursor.next.key, key) == 0)
            {
    ...
    public boolean remove(@NotNull KeyValuePair<TKey, TValue> keyValuePair)
    {
        ...
        // If the first node contains the item, make first point to first.next and decrease the count by 1.
        if ((keyComparator.compare(first.key, keyValuePair.getKey()) == 0) &&
                (valueComparator.compare(first.value, keyValuePair.getValue()) == 0))
        {
            first = first.next;
            count--;
            version++;
        ...
        for (Node precursor = first; precursor.next != null; precursor = precursor.next)
        {
            if ((keyComparator.compare(precursor.next.key, keyValuePair.getKey()) == 0) &&
                    (valueComparator.compare(precursor.next.value, keyValuePair.getValue()) == 0))
            {
                precursor.next = precursor.next.next;
                count--;
                version++;
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        dataworks.tests.DictionaryTest.runTest(dictionary);
    }
}
